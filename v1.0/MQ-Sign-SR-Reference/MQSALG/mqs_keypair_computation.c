...
void cal_ext_pk_mqsr_ref(ext_pk* Qs, const sk_mqsr* Fs, const sk_mqsr* Ts)
{
#if defined(_MALLOC_)
	...
	batch_mat_madd(tempFq2, Fs->Fq2, _V, Ts->vec_t + _V , _O, 1, 1, _O); // F2*to
...
void cal_ext_pk_mqsr(ext_pk* Qs, const sk_mqsr* Fs, const sk_mqsr* Ts)
{
	cal_ext_pk_mqsr_impl(Qs, Fs, Ts);
}